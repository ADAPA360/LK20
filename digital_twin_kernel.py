...
Production CPU-first governed digital-twin substrate for the Akkurat stack.

This file is the core world-model runtime used by:
...
Design constraints
------------------
- This kernel owns topology, state, governance, projection, fusion, history,
  Merkle verification, persistence, and policy-gated actions.
- tn.py owns low-level TensorTrain / Tucker tensor operations.
- twin_anything.py owns universal object/process branch materialization.
- lk20_kernel.py owns LK20 curriculum-domain materialization.

Public API required by dependent scripts
----------------------------------------
- NodeKind
- FusionMode
- LatentGeometry
...
DIGITAL_TWIN_KERNEL_VERSION = "digital-twin-kernel-v4.0-akkurat-lk20-tn31"
...
def configure_paths(*, project_root: Optional[Union[str, os.PathLike]] = None) -> Dict[str, Optional[str]]:
    ...
    for candidate in candidates:
        ...
        if found_self is None and (p / "digital_twin_kernel.py").exists():
            found_self = p
    ...
    return {
        ...
        "digital_twin_kernel_root": None if found_self is None else str(found_self),
    }
...
class TensorNetworkError(Exception):
    """Raised for invalid topology, state, projection, or tensor operations."""
...
class FederationError(Exception):
    """Raised for federation snapshot or trust-contract errors."""
...
def _coerce_geometry(v: Any, *, geometry: str = "euclidean", eps: float = _EPS) -> np.ndarray:
    g = str(geometry or "euclidean").lower().strip()
    if g in {"hyperbolic", "poincare", "poincaré"}:
...
class ProjectionEngine:
    """Deterministic embeddings for numeric arrays, JSON, text, and optional external embeddings."""

    def __init__(self, cfg: ProjectionConfig):
        self.cfg = cfg
        self._tt_cache: Dict[Tuple[str, int, int, int], Any] = {}

    def _finish(self, v: Any) -> np.ndarray:
        return _coerce_geometry(v, geometry=self.cfg.latent_geometry, eps=self.cfg.eps)

    def _factor_modes(self, n: int) -> List[int]:
        n = int(max(1, n))
        modes = int(max(1, self.cfg.tt_num_modes))

        if _TN_OK and tn is not None and hasattr(tn, "factorize_into_modes"):
            try:
                return [int(x) for x in tn.factorize_into_modes(n, modes)]
            except Exception:
                pass
        ...
        if _TN_OK and tn is not None and hasattr(tn, "get_factors"):
            try:
                a, b = tn.get_factors(n)
                return [int(a), int(b)]
            except Exception:
                pass
    ...
    def _tn_project(self, x: Any, *, key: str) -> np.ndarray:
        ...
        if not hasattr(tn, "TensorTrain") or not hasattr(tn, "TTConfig"):
            return self._hash_project(x, key=key)
        ...
        if op is None:
            try:
                output_dims = self._factor_modes(D)
                input_dims = self._factor_modes(L)
                num_cores = len(output_dims)

                if len(input_dims) != num_cores:
                    output_dims = self._factor_modes(D)
                    input_dims = self._factor_modes(L)
                ...
                bond_dims = [R for _ in range(max(0, len(output_dims) - 1))]

                rng = _rng_from_key(self.cfg.seed, f"ttproj::{key}::{L}->{D}::{R}")
                tt_cfg = tn.TTConfig(dtype=np.float32, device="cpu", check_finite=True)
        ...
        try:
            y = np.asarray(op.apply(arr), dtype=np.float32).reshape(-1)
    ...
    def encode_numbers(self, arr: Any, *, key: str) -> np.ndarray:
        ...
        feat = np.concatenate([x[: min(512, x.size)], stats], axis=0)
    ...
    def encode_json(self, obj: Any, *, key: str) -> np.ndarray:
        ...
        feat = np.concatenate([_l2_normalize(buckets), length_stats], axis=0)
        return self.project(feat, key=f"json::{key}")
...
class RingSketch:
    ...
    def push(self, sketch: Any, *, ts: Optional[int] = None) -> None:
        ...
        if s.size != int(self.cfg.sketch_dim):
            out = np.zeros(int(self.cfg.sketch_dim), dtype=np.float32)
            for i, val in enumerate(s):
                h = _stable_hash_u32(f"sketch_resize::{i}")
                out[h % out.size] += np.float32(val)
            s = _l2_normalize(out)
    ...
    def get_recent_pair(self, lag: int = 1, k: int = 32) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if S.shape[0] <= lag:
            z = np.zeros((0, int(self.cfg.sketch_dim)), dtype=np.float32)
            return z, z
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> "RingSketch":
        ...
        base = SketchConfig()
        cfg = SketchConfig(
            sketch_dim=int(cfgd.get("sketch_dim", base.sketch_dim)),
            capacity=int(cfgd.get("capacity", base.capacity)),
            use_float16=bool(cfgd.get("use_float16", base.use_float16)),
        )
...
class AdaptiveBaselineTracker:
    ...
    def snapshot(self) -> Dict[str, float]:
        return {
            "mu": float(self.mu),
            "sigma": float(math.sqrt(max(float(self.cfg.eps), float(self.var)))),
            "n": float(self.n),
            "z": 0.0,
            "beta": float(self.cfg.base_beta),
        }
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> "AdaptiveBaselineTracker":
        ...
        cfg = BaselineConfig(
            base_beta=float(cfgd.get("base_beta", base.base_beta)),
            beta_min=float(cfgd.get("beta_min", base.beta_min)),
            beta_max=float(cfgd.get("beta_max", base.beta_max)),
            z_open=float(cfgd.get("z_open", base.z_open)),
            z_harden=float(cfgd.get("z_harden", base.z_harden)),
            eps=float(cfgd.get("eps", base.eps)),
        )
...
class MerkleHasher:
    ...
    def hash_leaf(self, node_id: str, sketch: Any) -> int:
        ...
        seed_bytes = int(self.seed & 0xFFFFFFFF).to_bytes(4, "little", signed=False)
        return _bytes_hash_u64(sid + sk + seed_bytes)
...
class CausalProbe:
    ...
    def suggest(
        self,
        histories: Mapping[str, RingSketch],
        ...
        if candidates:
            ...
            top = [
                nid
                for nid, hist in sorted(histories.items())
                if hist.size() >= self.cfg.min_samples and nid.count(".") <= 3
            ]
        ...
        for a, b in pairs:
            ...
            if score >= self.cfg.threshold:
                out.append(SuggestEdge(src=a, dst=b, score=score, note="src_past_predicts_dst_now"))
            elif score <= -self.cfg.threshold:
                out.append(SuggestEdge(src=b, dst=a, score=-score, note="dst_past_predicts_src_now"))

        out.sort(key=lambda e: float(e.score), reverse=True)
        return out[: int(self.cfg.max_suggestions)]

    def suggest_cross_twin(
        self,
        local_histories: Mapping[str, RingSketch],
        peer_histories: Mapping[str, Any],
        *,
        ...
        local_keys = [
            k
            for k, h in local_histories.items()
            if h.size() >= self.cfg.min_samples and k.count(".") <= 3
        ]

        for lk in local_keys:
            L_p, L_n = local_histories[lk].get_recent_pair(lag=self.cfg.lag, k=self.cfg.k)

            for pk in sorted(peer_histories.keys()):
                ...
                if score >= self.cfg.threshold:
                    candidates.append(
                        SuggestEdge(
                            src=lk,
                            dst=f"{peer_prefix}:{pk}",
                            score=score,
                            note="local_past_predicts_peer_now",
                        )
                    )
                elif score <= -self.cfg.threshold:
                    candidates.append(
                        SuggestEdge(
                            src=f"{peer_prefix}:{pk}",
                            dst=lk,
                            score=-score,
                            note="peer_past_predicts_local_now",
                        )
                    )
    ...
    def _coerce(self, v: Any) -> np.ndarray:
        ...
        if x.size != int(self.vector_dim):
            ...
            for i, val in enumerate(x):
                h = _stable_hash_u32(f"node_resize::{self.node_id}:{i}")
...
class _TuckerFusionAdapter:
    def __init__(
        ...
        *,
        ...
        self.tucker_rank = int(max(1, tucker_rank))
        self.projector = projector
        self.key = str(key)
        self._layer = None

        if _TN_OK and tn is not None and hasattr(tn, "TuckerFusionLayer"):
            try:
                rng = _rng_from_key(seed, f"tn_tucker::{key}::{vector_dim}->{tucker_rank}")
                self._layer = tn.TuckerFusionLayer(
    ...
    def fuse_pair(self, a: Any, b: Any) -> np.ndarray:
        ...
        return self.projector.project(
            ...
            key=f"tn_tucker_fallback::{self.key}",
        )
    ...
    *,
    ...
    if mode == FusionMode.SUM:
        ...
        if adapter_cache is not None and ck in adapter_cache:
            ...
            adapter = _TuckerFusionAdapter(
                vector_dim=D,
                tucker_rank=int(cfg.tucker_rank),
                seed=projector.cfg.seed,
                key=fusion_key,
                projector=projector,
            )
        ...
        r = int(max(1, min(int(cfg.tucker_rank), D)))
        Z = []
...
class TreeTensorNetwork:
    def __init__(
        ...
        *,
        ...
        if self.latent_geometry not in {LatentGeometry.EUCLIDEAN.value, LatentGeometry.HYPERBOLIC.value, "poincare", "poincaré"}:
            self.latent_geometry = LatentGeometry.EUCLIDEAN.value

        if self.latent_geometry in {"poincare", "poincaré"}:
            self.latent_geometry = LatentGeometry.HYPERBOLIC.value
        ...
        self.tt_rank = int(max(1, tt_rank))
        ...
        self.sketch_projector = ProjectionEngine(
            ProjectionConfig(
                vector_dim=self.sketch_dim,
                seed=self.seed,
                use_tn_projection=self.use_tn_projection,
                tt_rank=max(1, min(self.tt_rank, 4)),
                max_in_for_tn=4096,
                latent_geometry=LatentGeometry.EUCLIDEAN.value,
            )
        )
    ...
    def add_node(
        ...
        *,
        ...
        if node_id in self.nodes:
            if metadata:
                self.nodes[node_id].metadata.update(dict(metadata))
                self.bump_tick()
            return
        ...
        self.histories[node_id] = RingSketch(
            SketchConfig(
    ...
    def remove_node(self, node_id: str, *, recursive: bool = False) -> None:
        ...
        if node_id == self.root_id:
            raise TensorNetworkError("Cannot remove root node.")

        if node_id not in self.nodes:
            return

        kids = list(self.children.get(node_id, []))

        if kids and not recursive:
            raise TensorNetworkError(f"Node {node_id} has children; pass recursive=True.")

        for k in kids:
            self.remove_node(k, recursive=True)

        parent = self.parent.get(node_id)
        if parent and parent in self.children:
            self.children[parent] = [x for x in self.children[parent] if x != node_id]

        self.nodes.pop(node_id, None)
        self.children.pop(node_id, None)
        self.parent.pop(node_id, None)
        self.histories.pop(node_id, None)
        self.baselines.pop(node_id, None)
    ...
    def update_node_data(
        ...
        data: Any,
        *,
        cortex: Optional[CortexLike] = None,
        ...
        if self.control_plane:
            self.control_plane.log_event(f"Node {node_id} updated from data.", meta={"note": note})
    ...
    def measure(self, node_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "merkle": MerkleHasher.hex64(self.merkle_root(node_id)),
        }
    ...
    def sandbox_clone(self, suffix: str = "sandbox") -> "TreeTensorNetwork":
        ...
        for nid, node in self.nodes.items():
            fusion = FusionConfig(
                mode=FusionMode(str(node.fusion.mode)),
                attention_beta=float(node.fusion.attention_beta),
                residual_mix=float(node.fusion.residual_mix),
                tucker_rank=int(node.fusion.tucker_rank),
                auto_tune_beta=bool(node.fusion.auto_tune_beta),
                learnable_bias=float(node.fusion.learnable_bias),
                eps=float(node.fusion.eps),
            )
            ...
            clone.histories[nid] = RingSketch(
                SketchConfig(
                    sketch_dim=self.sketch_dim,
                    capacity=self.history_capacity,
                    use_float16=True,
    ...
    def export_history_snapshot(
        ...
        *,
        ...
        histories = {
            nid: self.histories[nid].get_recent(k).astype(float).tolist()
            for nid in ids
            if nid in self.histories
        }
    ...
    def verify_history_snapshot(snapshot: TwinHistorySnapshot, *, max_age_s: int = 300) -> Dict[str, Any]:
        ...
        return {
            ...
            "peer_id": snapshot.peer_id,
            "root_hash": snapshot.root_hash,
            "node_count": len(snapshot.histories),
        }
    ...
    def to_dict(self, *, include_histories: bool = True) -> Dict[str, Any]:
        return {
            "version": DIGITAL_TWIN_KERNEL_VERSION,
            "vector_dim": int(self.vector_dim),
            "seed": int(self.seed),
    ...
    def save_json(self, path: Union[str, os.PathLike], *, include_histories: bool = True) -> None:
        _atomic_write_text(
            path,
            json.dumps(_json_safe(self.to_dict(include_histories=include_histories)), indent=2, ensure_ascii=False),
        )
    ...
    def from_dict(cls, payload: Mapping[str, Any]) -> "TreeTensorNetwork":
        ttn = cls(
            ...
            tt_rank=int(payload.get("tt_rank", 8)),
        )
        ...
        for nid in ttn.nodes:
            ...
            ttn.histories[nid] = RingSketch(
                SketchConfig(
                    sketch_dim=ttn.sketch_dim,
                    capacity=ttn.history_capacity,
                    use_float16=True,
                )
            )
...
class PolicyEngine:
    ...
    def simulate(
        ...
        *,
        ...
        ok = bool(
            predicted_alerts <= int(self.cfg.dreaming_fail_threshold)
            and max_delta <= float(self.cfg.max_latent_l2_delta)
        )
...
class AkkuratInterface:
    ...
    def _rule_plan(self, intent: str) -> List[Action]:
        ...
        if any(k in it for k in ["stabilize", "resync", "synchronize", "reduce drift"]):
            return [
                Action("raise_alert", payload="Requested stabilization workflow.", note="operator_attention"),
                Action("attach_metadata", node_id="1.2", payload={"requested": "recalibration"}, note="virtual_space"),
            ]
        ...
        if "curriculum" in it or "lk20" in it:
            return [Action("attach_metadata", node_id="7.0", payload={"requested": intent}, note="lk20")]
    ...
    def plan(self, intent: str, *, allow_llm: bool = True) -> List[Action]:
        ...
        system = (
            "You are an operations planner for a governed digital twin. "
            "Return ONLY JSON: "
            "{\"actions\":[{\"kind\":...,\"node_id\":...,\"payload\":...,\"note\":...}]}"
        )
        user = f"INTENT: {it}\nKNOWN_NODE_IDS: {', '.join(sorted(self.network.nodes.keys()))[:4000]}\nPlan safe actions."
        ...
        try:
            ...
            self.log_event("plan() LLM failed", meta={"intent": it, "error": repr(exc)})
            ...
            if isinstance(a, Mapping):
                out.append(
                    Action(
                        ...
                        node_id=(str(a.get("node_id")).strip() if a.get("node_id") is not None else None),
                        payload=a.get("payload", None),
    ...
    def execute(self, actions: List[Action]) -> Dict[str, Any]:
        ...
        for act in actions[: int(self.policy.cfg.max_actions_per_execute)]:
            try:
                ...
                stats["errors"] += 1
                detail = {"kind": act.kind, "node_id": act.node_id, "error": repr(exc)}
    ...
    def build_platform(
        *,
        ...
        ttn = TreeTensorNetwork(
            ...
            seed=seed,
            root_name="Akkurat Governed Digital Twin",
        ...
        # Core Akkurat platform substrate.
        ...
        ttn.add_node("2.1.2", "Machines / devices", parent_id="2.1", kind=NodeKind.ENTITY)
        ttn.add_node("2.1.3", "Infrastructure", parent_id="2.1", kind=NodeKind.ENTITY)
        ttn.add_node("2.1.4", "Operators / people", parent_id="2.1", kind=NodeKind.ENTITY)
        ...
        ttn.add_node("2.3.1", "Workspace / lab / environment", parent_id="2.3", kind=NodeKind.RESOURCE)
        ttn.add_node("2.3.2", "Sensors / observability", parent_id="2.3", kind=NodeKind.SENSOR)
        ...
        ttn.add_node("3.1.3", "Computational models", parent_id="3.1", kind=NodeKind.MODEL)
        ...
        # Cognitive runtime branch.
        ttn.add_node("3.2.3.4", "Akkurat cognitive lobe runtime", parent_id="3.2.3", kind=NodeKind.COGNITIVE)
        ...
        # Data branch.
        ttn.add_node("4.1", "Data processing pipeline", parent_id="1.3", kind=NodeKind.PIPELINE)
        ...
        ttn.add_node("4.2.4", "Visualization / VR / AR integration", parent_id="4.2", kind=NodeKind.VISUAL)
        ...
        # Reserved roots used by dependent adapters.
        ttn.add_node(
            "5.0",
            "Thing Twin Reserved Root",
            parent_id="0",
            kind=NodeKind.GENERIC,
            metadata={
                "reserved_for": "twin_anything.py",
                "content_status": "empty_until_materialized",
            },
        )
        ttn.add_node(
            "7.0",
            "LK20 Curriculum Twin Reserved Root",
            parent_id="0",
            kind=NodeKind.GOVERNANCE,
            metadata={
                "reserved_for": "lk20_kernel.py",
                "content_status": "empty_until_materialized",
            },
        )

        # Fusion tuning.
        ttn.set_fusion("0", FusionConfig(mode=FusionMode.ATTENTION, attention_beta=2.0, residual_mix=0.20))
        ttn.set_fusion("1.1", FusionConfig(mode=FusionMode.ATTENTION, attention_beta=2.2, residual_mix=0.25))
        ttn.set_fusion("1.2", FusionConfig(mode=FusionMode.ATTENTION, attention_beta=2.2, residual_mix=0.25))
        ttn.set_fusion("1.3", FusionConfig(mode=FusionMode.ATTENTION, attention_beta=2.0, residual_mix=0.25))
        ttn.set_fusion("3.2.3.4", FusionConfig(mode=FusionMode.ATTENTION, attention_beta=2.4, residual_mix=0.15))
        ttn.set_fusion("3.2.3.5", FusionConfig(mode=FusionMode.ATTENTION, attention_beta=2.4, residual_mix=0.15))

        if _TN_OK and tn is not None and hasattr(tn, "TuckerFusionLayer"):
            ttn.set_fusion("4.1.4", FusionConfig(mode=FusionMode.TN_TUCKER, tucker_rank=16, residual_mix=0.15))
        else:
            ttn.set_fusion("4.1.4", FusionConfig(mode=FusionMode.TUCKER, tucker_rank=16, residual_mix=0.15))

        # Seed metadata into root.
        ttn.nodes["0"].metadata.update(
            {
                "kernel_version": DIGITAL_TWIN_KERNEL_VERSION,
                "tn": tn_status(),
                "created_at": _now_iso(),
                "adapter_roots": {
                    "twin_anything": "5.0",
...
def run_automation_demo(
    *,
    ...
    print(json.dumps(_json_safe(status_report()), indent=2, ensure_ascii=False))

    ttn = DigitalTwinsBuilder.build_platform(
        vector_dim=vector_dim,
        seed=seed,
        sketch_dim=min(96, vector_dim),
        history_capacity=64,
        latent_geometry=latent_geometry,
        use_tn_projection=True,
    )
    cp = AkkuratInterface(ttn, cortex=cortex, strict=False)

    print(f"[SYSTEM] Nodes: {len(ttn.nodes)} | vector_dim={vector_dim} | sketch_dim={ttn.sketch_dim}")
    ...
    ttn.update_node_data(
        "4.1.4",
        {
            "sensor": sensor_readings.tolist(),
            "ts": _now_iso(),
            "note": "bounded fusion artefact",
        },
        cortex=cortex,
        note="fusion",
    )
    ...
    print(f"[SYSTEM] Causal suggestions: {len(ttn.causal.suggest(ttn.histories))}")

    gov = cp.governed_execute("stabilize and resync the virtual model with physical data", allow_llm=False)
    print(json.dumps(_json_safe(gov), indent=2, ensure_ascii=False))

    snap = ttn.export_snapshot(peer_id="peer_demo", node_id="0")
    contract = TwinContract("peer_demo", policy=ContractPolicy())
    print(json.dumps(_json_safe(contract.ingest(ttn.sketch("0"), snap)), indent=2, ensure_ascii=False))
